 ...
 */
class AskArticleModel @Inject constructor(private val repository: AskAndSquareArticleRepository)
    : ViewModel(){

    val loadState = MutableLiveData<PagingRetryLoad>()

    fun loadAskArticles() = repository.loadAskArticles().cachedIn(viewModelScope)
